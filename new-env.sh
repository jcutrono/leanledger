...
echo "setup S3"
ssh ubuntu@$dns << EOF
dokku mongo:backup-auth leanledger {api key} {secret key}
exit
EOF

echo "backup previous environment"
current_prod_dns=$(aws ec2 describe-instances --filters Name=tag:Name,Values=LeanLedger-Prod --query 'Reservations[].Instances[].PublicDnsName' --output text)
...
current_prod_instanceid=$(aws ec2 describe-instances --filters Name=tag:Name,Values=LeanLedger-Prod --query 'Reservations[].Instances[].InstanceId' --output text)
aws ec2 create-tags --resources $current_prod_instanceid --tags Key=Name,Value=LeanLedger-Prod-old
aws ec2 stop-instances --instance-ids $current_prod_instanceid

echo "rename Prod-Cli to Prod"
aws ec2 create-tags --resources $instance_id --tags Key=Name,Value=LeanLedger-Prod
